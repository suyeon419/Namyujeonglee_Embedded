# ⏰ 옴짝달싹
&nbsp;옴짝달싹은 뽀모도로 공부법을 사용한 타이머로, 대표 기능으로 타이머 기능과 무드등 기능이 있다.

&nbsp;뽀모도로 공부법이란, 25분 동안 공부에 집중하고 5분 동안 쉬는 것을 3~4번 반복하는 공부법으로 공부의 효율을 높이기 위해 사용된다. 여기서 25분 동안 공부해야한다는 제한이 있다는 점이 아쉽다고 생각을 하여 본 프로젝트에서는 다양한 시간을 제공하여 사용자에게 맞는 공부 시간으로 뽀모도로 공부법을 활용할 수 있도록 하였다.

&nbsp;타이머 기능에는 25분, 35분, 45분으로 공부 시간을 설정할 수 있고 쉬는 시간은 10분으로 고정하였다. 뽀모도로 공부법을 따라 공부 사이클은 총 3번 돌아가게 되는데, 공부 시간이 끝나면 파란 불이 3번 점멸하고 쉬는 시간이 끝나면 빨간 불이 3번 점멸하여 사용자에게 시각적으로 알려준다. 이때 알람 소리가 나지 않는데, 이는 조용한 독서실에서도 사용할 수 있도록 의도한 것이다. 공부 시간 중에 사용자가 자리에 없으면 시계 바늘이 일시 정지 되면서 시간이 멈추게 되고 사용자가 자리에 돌아오면 다시 남은 시간과 사이클이 돌아가도록 하였다. 쉬는 시간일 때는 사용자가 자리에 없더라도 시간이 흘러가도록 설계하였다.

&nbsp;무드등 기능은 현재 공간의 밝기에 따라 눈에 편한 주황계 색상의 LED가 2단계 밝기기로 조절되도록 하였다.
...
- LED 제어
    - 주변 빛이 어두우면 LED를 밝히고, 주변 빛이 밝으면면 LED를 어둡게 한다.
...
&nbsp;초음파 센서는 앞쪽에 배치되어 있으며, 사용자가 자리에 앉아있는지 판별할 때 사용된다. 구멍은 초음파 센서를 안정적으로 고정시키는 역할을 하며, 센서와 연결된 전선을 외부로 쉽게 빼낼 수 있도록 디자인되었다. 
...
# 사용 설명서
1. 전원 연결 후 무드등의 색깔이 주황빛으로 변할 때까지 대기한다.
2. 원하는 버튼을 눌러 타이머를 작동 시킨다
   - 타이머를 강제 종료하고 싶다면 눌렀던 버튼을 다시 눌러 강제 종료 시킬 수 있다.
   - 공부 중 자리에서 이동을 한다면 타이머가 일시 중지되어 시간이 흐르지 않는다. 그러나 쉬는 시간일 때는 자리에서 이동하더라도 일시 정지가 안 된다.
3. 3번의 사이클이 끝난 후 공부를 더 하고 싶다면 원하는 버튼을 눌러 다시 시작할 수 있다.
