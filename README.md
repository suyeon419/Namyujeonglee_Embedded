# ⏰ 옴짝달싹
&nbsp; 옴짝달싹의 대표 기능으로 타이머 기능과 무드등 기능이 있다.

&nbsp; 타이머 기능에는 25분, 35분, 45분으로 공부 시간을 설정할 수 있다. 뽀모도로의 기법을 따라 공부 사이클은 총 3번 돌아가게 되는데, 공부 시간이 끝나면 파란 불이 3번 점멸하고 쉬는 시간이 끝나면 빨간 불이 3번 점멸하여 사용자에게 시각적으로 알려준다. 이때 알람 소리가 나지 않는데, 이는 조용한 독서실에서도 사용할 수 있도록 의도한 것이다. 공부 시간 중에 사용자가 자리에 없으면 시계 바늘이 일시 정지 되면서 시간이 멈추게 되고 사용자가 자리에 돌아오면 다시 남은 시간과 사이클이 돌아가도록 하였다. 쉬는 시간일 때는 사용자가 자리에 없더라도 시간이 흘러가도록 설계하였다.

&nbsp; 무드등 기능은 현재 공간의 밝기에 따라 무드등의 밝기가 2단계로 조정되도록 하였다.
